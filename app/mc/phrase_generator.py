...
from app.mc import const
from app.mc.markov_chain import MarkovChain
from app.mc.types import Formatter, Validator
...
class PhraseGenerator:
    ...
    async def generate_phrases(
        ...
        for _ in range(count):
            try:
                phrase = await self.generate_phrase(attempts, validators, formatters)
                phrases.append(phrase)
            except PhraseGeneratorError:
                raise
        return phrases

    async def generate_phrase_or_none(
        self,
        attempts: int = 999,
        validators: Optional[List[Validator]] = None,
        formatters: Optional[List[Formatter]] = None,
        ...
        try:
            return await self.generate_phrase(attempts, validators, formatters)
        except PhraseGeneratorError:
            return None
    ...
    async def generate_phrase(
        ...
        for _ in range(attempts):
            current_frame = await self.model.get_full_frame(beginning=(const.START,))

            words = [word for word in current_frame]
            words.append(await self.model.get_random_available_word(frame=current_frame))

            while const.END not in words:
                phrase_len = len(words)
                current_frame = await self.model.get_full_frame(
